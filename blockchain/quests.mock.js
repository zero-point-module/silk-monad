/**
 * FILE-BACKED MOCK of the quests.js bridge — for parallel/offline development.
 *
 * Same exported signatures as the real blockchain/quests.js, but no chain: state
 * lives in ./quests-mock.json (cwd-relative) so it works ACROSS the separate
 * per-agent Node processes — the quest master process creates a quest and the
 * player processes can see + claim it. It uses the real viem keccak so the
 * answer-hash convention is identical to on-chain; only the "chain" is faked.
 *
 * To run the agents offline against this mock, point the imports in
 * agent-backend/src/agent/commands/{actions,queries}.js at './quests.mock.js'.
 *
