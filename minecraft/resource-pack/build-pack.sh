...
echo "SHA-1  : $SHA1"
echo "URL    : https://raw.githubusercontent.com/zero-point-module/silk-monad/main/minecraft/resource-pack/pack.zip"
echo
echo "Next   : update RESOURCE_PACK_SHA1 in silk-monad/minecraft/server/docker-compose.yml to:"
echo "         $SHA1"
echo "         then: git add -A && git commit && git push && (cd ../server && docker compose up -d --force-recreate)"
