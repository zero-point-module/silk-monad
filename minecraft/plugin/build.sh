...
# If --install is passed, also copies the jar into ../server/data/plugins/.
...
if [[ "${1:-}" == "--install" ]]; then
  DEST="../server/data/plugins/SilkMonad.jar"
  cp "$JAR" "$DEST"
  echo "Installed to: $DEST"
  echo "Restart the server: (cd ../server && docker compose restart minecraft)"
fi
